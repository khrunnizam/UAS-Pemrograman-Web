<script>
  ...
  async function fetchCategories() {
    ...
    try {
      ...
      toastStore.error(err.message || 'Gagal memuat data kategori');
    } finally {
      isLoading = false;
    }
  }
  ...
  function openEditModal(category) {
    isEditMode = true;
    form = { id: category.id, nama: category.nama };
    formErrors = {};
    showModal = true;
  }
  ...
  function validateForm() {
    ...
    if (!form.nama.trim()) {
      ...
      errors.nama = 'Nama kategori maksimal 100 karakter';
    }
    formErrors = errors;
  ...
  async function handleSubmit(event) {
    ...
    try {
      if (isEditMode) {
        await categoryService.update(form.id, { nama: form.nama.trim() });
        toastStore.success('Kategori berhasil diperbarui ✅');
      } else {
        await categoryService.create({ nama: form.nama.trim() });
        toastStore.success('Kategori berhasil ditambahkan ✅');
      }
      closeModal();
      await fetchCategories();
    } catch (err) {
      toastStore.error(err.message || 'Gagal menyimpan kategori');
    } finally {
      isSubmitting = false;
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toastStore.success(`Kategori "${itemToDelete.nama}" berhasil dihapus`);
      closeDeleteConfirm();
      await fetchCategories();
    } catch (err) {
      toastStore.error(err.message || 'Gagal menghapus kategori');
    } finally {
      isSubmitting = false;
    }
  }
...
<svelte:head>
  <title>Daftar Kategori – Perpustakaan Umum Daerah</title>
  <meta name="description" content="Kelola kategori buku perpustakaan: tambah, edit, dan hapus kategori." />
</svelte:head>
  ...
  <div class="page-container">
    ...
    <div class="page-header">
      <div class="page-header__left">
        ...
        <h1 class="page-header__title">Manajemen Kategori</h1>
        <p class="page-header__subtitle">Kelola kategori buku perpustakaan</p>
      </div>
      <button class="btn btn--primary" id="add-category-btn" onclick={openAddModal}>
        + Tambah Kategori
      </button>
    </div>
    ...
    <div class="nav-tabs">
      ...
      <a href="/categories" class="nav-tab nav-tab--active" id="tab-categories">🏷️ Kategori</a>
    </div>
    ...
    <div class="search-bar">
      ...
      {#if searchQuery}
        <button
          ...
          onclick={() => { searchQuery = ''; fetchCategories(); }}
      ...
      <div class="empty-state">
        ...
        <p class="empty-state__desc">
          {searchQuery
            ...
            : 'Tambahkan kategori pertama ke sistem perpustakaan.'}
        </p>
        {#if !searchQuery}
          <button class="btn btn--primary" onclick={openAddModal}>+ Tambah Kategori Pertama</button>
        {/if}
      </div>
    {:else}
      <div class="list-info">
        <span class="list-info__count">
          {categories.length} kategori {searchQuery ? `ditemukan untuk "${searchQuery}"` : 'terdaftar'}
        </span>
      </div>
            ...
            {#each categories as category, i (category.id)}
              <tr class="data-table__row">
                ...
                <td class="data-table__name">
                  <span class="name-text">🏷️ {category.nama}</span>
                </td>
...
{#if showModal}
  <div class="modal-overlay" onclick={closeModal} role="dialog" aria-modal="true" aria-label={isEditMode ? 'Edit kategori' : 'Tambah kategori'}>
    <div class="modal" onclick={(e) => e.stopPropagation()}>
      <div class="modal__header">
        <h2 class="modal__title">{isEditMode ? '✏️ Edit Kategori' : '🏷️ Tambah Kategori'}</h2>
        <button class="modal__close" onclick={closeModal} aria-label="Tutup">×</button>
      </div>

      <form class="modal__form" id="category-form" onsubmit={handleSubmit} novalidate>
        <div class="form-group">
          <label for="category-nama" class="form-label">Nama Kategori</label>
          <input
            id="category-nama"
            type="text"
            class="form-input"
...
{#if showDeleteConfirm && itemToDelete}
  <div class="modal-overlay" role="dialog" aria-modal="true" aria-label="Konfirmasi hapus kategori">
    <div class="modal modal--sm">
      <div class="modal__header">
        <h2 class="modal__title">🗑️ Hapus Kategori</h2>
      ...
      <div class="delete-confirm__body">
        ...
        <p class="delete-confirm__warning">
          ⚠️ Kategori tidak dapat dihapus jika masih memiliki buku terdaftar.
        </p>
      </div>
      <div class="modal__footer">
...
<style>
  ...
  .data-table__row:hover { background: rgba(192, 132, 252, 0.04); }
  ...
  .modal__title { font-size: 1.25rem; font-weight: 800; color: #f1f5f9; }
  ...
  .form-label { font-size: 0.875rem; font-weight: 600; color: #94a3b8; }
  ...
  .delete-confirm__name { color: #f1f5f9; font-weight: 700; font-size: 1rem; }
  .delete-confirm__warning { color: #fcd34d; font-size: 0.85rem; margin-top: 0.25rem; }
